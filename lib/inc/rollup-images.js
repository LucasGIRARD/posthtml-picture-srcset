  ...
  .then((webpImage) => Promise.allSettled(
    (outputs)
      ...
      .map((scaleWidth, i) => {
        ...
        try{
          var buf = fs.readFileSync(outputWebp);
          imageDimensions = imageSize(buf);
        } catch(e){
          console.error(`Error reading dimensions of image ${outputWebp}:`, e);
        }
        ...
        var imagePathPost = imagePathPre;
...
const generateAll = (options) => {
  ...
  return new Promise((resolve) => {
    ...
    globby__default["default"].globby(`${dirGlob}/**/*.{${inputFormats.join(',')}}`)
      .then((images) => {

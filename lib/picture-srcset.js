...
export default function plugin(options = {}) {
  ...
  //const imgDir = options.imgDir || './dist/assets'
  ...
  const getPicture = async (img, src) => {
    ...
    //var dimensions = await imageSizeFromFile(src)
    var relativeSrc = src.split(cwd+'\\')[1]
    relativeSrc = relativeSrc.replace(/\\/g, '/'); // Normalize path for web usage
    ...
    if (typeof (img['attrs']['fullsize']) == "undefined") {
      ...
      for (var i = 0; i < imageOptions.size.length; i++) {
        ...
        var image =
          ...
          relativeSrc.replace(/\.(\w+)$/, suffix + '.webp') +
          ' ' +
          size +
          'w'
